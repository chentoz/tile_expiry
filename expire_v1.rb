...
module Expire
  ...
  HASH_ROOT = "/var/lib/mod_tile/ajt"
    ...
    doc.find('//way/nd').each do |node|
      ...
      unless nodes.include? node_id
        ...
        if res.values.empty?
          ...
          lon = res.values[0][0].to_f / 10000000
          lat = res.values[0][1].to_f / 10000000

          # chentoz : original way of processing
          # point = Proj4::Point.new(res.values[0][0].to_f / 100.0, res.values[0][1].to_f / 100.0)
          # nodes[node_id] = tile_from_merc(point, MAX_ZOOM)

          point = Proj4::Point.new(Math::PI * lon / 180,
            Math::PI * lat / 180)
          nodes[node_id] = tile_from_latlon(point, MAX_ZOOM)
        end
      end
    end
